...
describe("limitToTwoDecimals", () => {
  ...
  it("acepta números con 1 decimal", () => {
    expect(limitToTwoDecimals("12.3")).toBe("12.3");
    expect(limitToTwoDecimals("12,3")).toBe("12,3");
  });

  it("acepta 0 como valor", () => {
    expect(limitToTwoDecimals("0")).toBe("0");
  });

  it("maneja valores vacíos o no numéricos", () => {
    expect(limitToTwoDecimals("")).toBe("");        // vacío
    expect(limitToTwoDecimals("12a34")).toBe("12"); // corta al número válido
  });

  it("mantiene separador original al recortar decimales", () => {
    expect(limitToTwoDecimals("3,456")).toBe("3,45");
    expect(limitToTwoDecimals("3.456")).toBe("3.45");
  });
